...
const useEmptyRenderContainer = (placeholder) => {
    const emptyRenderContainer = () => React.createElement(Text, { className: emptyPlaceholderCn }, placeholder);
...
export const TableColumnSetup = (props) => {
    ...
    const slotBeforeListBody = filterable ? (React.createElement(TextInput, { size: "m", view: "clear", placeholder: filterPlaceholder, value: filterState.filter, className: filterInputCn, onUpdate: onFilterValueUpdate, hasClear: true })) : null;
    const renderContainer = filterState.filter && !filterState.items.length ? emptyRenderContainer : dndRenderContainer;
    return (React.createElement(TreeSelect, { className: b(null, className), mapItemDataToProps: mapItemDataToProps, multiple: true, size: "l", open: open, value: value, items: filterState.filter ? filterState.items : items, onUpdate: onUpdate, popupWidth: popupWidth, onOpenChange: onOpenChange, placement: popupPlacement, slotBeforeListBody: slotBeforeListBody, renderContainer: renderContainer, renderControl: renderControl, renderItem: dndRenderItem }));
};
