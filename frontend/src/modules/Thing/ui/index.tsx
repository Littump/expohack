  ...
  return (
    <div className="flex flex-col gap-[20px]">
      ...
      <div className="flex gap-[20px] flex-col mt-[40px] items-start">
        ...
        <Modal buttonContent="Получить" title="Персональная рекомендация">
          <button
            onClick={() => {
              navigator.clipboard.writeText(scriptText ?? "");
            }}
            className="active:scale-90 flex gap-2 items-center hover:bg-gray-100 transition rounded-lg w-fit px-4 py-2 cursor-pointer"
          >
            Скопировать{" "}
            <svg
              xmlns="http://www.w3.org/2000/svg"
          ...
          {!scriptIsPending && scriptText
            ? scriptText
            : "Не удалось получить скрипт"}
        </Modal>
      </div>
    </div>
  );
